...
import { Grid, Checkbox, Box, Fade, Popper, ClickAwayListener, Popover, Paper, Stack, Typography, Switch, OutlinedInput, FormControl, InputLabel, Button, Chip, Autocomplete, TextField } from '@mui/material';
...
const TagsInput = ({ value, setValue }) => {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = itemstgs?.map((n) => n?.name);
                ...
                <Box>
                    <FormControl fullWidth>
                        ...
                        <OutlinedInput
                            name={'metatitle'}
                            value={selected}
                            onChange={(event) => { onChange(event) }} label={'Meta Title'}
                            readOnly={true}
                            ...
                            <Fade {...TransitionProps} timeout={350}>
                                <Paper sx={{
                                    ...
                                    <Box onScroll={(event) => { console.log('event', (event.target.scrollHeight - event.target.scrollTop === event.target.clientHeight)) }} style={{ display: "flex", flexDirection: "column", height: 200, overflow: "auto", }}>
                                        ...
                                        {itemstgs?.map((value, index) => {
                                            const isItemSelected = isSelected(value?.name);

                                            return (
                                                <Box onClick={() => handleClickSelect(value?.name)} key={index} sx={{ display: "flex", alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                                                    <Checkbox disableRipple checked={isItemSelected} onChange={() => { handleClickSelect(value?.name) }} />
                                                    <Typography variant='h6' sx={{ fontSize: "14px" }}>
                                                        {value?.name}
                                                    </Typography>
                                                </Box>
